...
// Already logged in as order manager?
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $conn->query("SELECT id, name, password 
                           ...
                           AND designation = 'ordermanager'");
    if ($check && $check->num_rows === 1) {
        $staff = $check->fetch_assoc();
        if ($password === $staff['password']) {
            $_SESSION['ordermanager_id']   = $staff['id'];
            $_SESSION['ordermanager_name'] = $staff['name'];
            unset($_SESSION['inserter_id'], $_SESSION['staff_id'], $_SESSION['staff_name']);
            header('Location: index.php');
            exit;
        } else {
        ...
        $error = 'No order manager account found with that email.';
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Order Manager Login | UpdatesAll</title>
...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="login-card bg-slate-800/90 backdrop-blur-xl border border-slate-700/50 rounded-3xl shadow-2xl p-8 w-full max-w-sm">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-amber-400 tracking-tight">Order Manager</h1>
            <p class="text-slate-400 text-sm mt-1">Sign in to manage orders</p>
        </div>
        <?php if ($error): ?>
        ...
        <form method="post" class="space-y-5">
            <div>
                ...
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-500"><i class="fas fa-envelope"></i></span>
            ...
            <div>
                ...
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-500"><i class="fas fa-lock"></i></span>
        ...
        <p class="text-xs text-slate-500 text-center mt-6 flex items-center justify-center gap-1">
            <i class="fas fa-shield-alt text-amber-500/50"></i> Order manager access only
        </p>
    </div>
    <script>
        ...
        if (errorDiv) errorDiv.addEventListener('animationend', () => errorDiv.classList.remove('error-shake'));
    </script>
</body>
</html>