...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['ajax'])) {
    ...
    // New action: return subcategories for a given category (for dynamic dropdown)
...
async function loadSubcategories(catId) {
    ...
    try {
        ...
        const res = await fetch('product.php', { method: 'POST', body: formData });
...
async function deleteExtraImage(imgId, slot) {
    ...
    try {
        ...
        await fetch('product.php', { method: 'POST', body: formData });
...
async function deleteProduct(id) {
    ...
    showLoader();
    try {
        const res = await fetch('product.php', { method: 'POST', body: new URLSearchParams({ ajax: 1, action: 'delete', id: id }) });
