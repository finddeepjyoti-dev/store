<?php
// admin/completeorder.php - All Orders with Search, Filters, and Delete

// ---------- AJAX Delete Order ----------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === 'delete_order') {
    error_reporting(0);
    ini_set('display_errors', 0);
    ob_clean();
    header('Content-Type: application/json');
    $orderId = intval($_POST['order_id']);
    // Delete order items first (if CASCADE is not set)
                ...
                <tr class="hover:bg-slate-700/40 transition-colors order-row"
                    ...
                    <td class="p-4 text-right">
                        <div class="flex items-center justify-end gap-3">
                            <a href="order_detail.php?id=<?= $o['id'] ?>" class="text-amber-400 hover:text-amber-300 transition-colors" title="View Order">
                                <i class="fas fa-eye"></i>
                            </a>
                            <button onclick="deleteOrder(<?= $o['id'] ?>)" class="text-red-400 hover:text-red-300 transition-colors" title="Delete Order">
...
async function deleteOrder(orderId) {
    if (!confirm('Are you sure you want to permanently delete this order? This will also remove its items.')) return;
    showLoader();
    try {
        const formData = new URLSearchParams();
        formData.append('ajax', 'delete_order');
        formData.append('order_id', orderId);
        const res = await fetch('completeorder.php', { method: 'POST', body: formData });
        const data = await res.json();
        if (data.success) {
            // Remove the row from the table
            const row = document.querySelector(`tr[data-order-id="${orderId}"]`);
            if (row) row.remove();
            // Optionally show a toast (depends on your admin layout)
            alert(data.message);
        } else {
